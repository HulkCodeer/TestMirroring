...
extension UIView {
    ...
    internal func makeToast(_ message: String?, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
            ...
            printLog(out: "Error: Message, Title, Image all nil")
        } catch {}
    }
            
    internal func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        do {
    ...
    internal func makeToast(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, title: String?, image: UIImage?, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)?) {
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
    ...
    internal func makeToastActivity(_ position: ToastPosition) {
        // sanity
    ...
    internal func makeToastActivity(_ point: CGPoint) {
        // sanity
        guard objc_getAssociatedObject(self, &ToastKeys.activityView) as? UIView == nil else { return }
    ...
    internal func toastViewForMessage(_ message: String?, title: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
        // sanity
...
internal struct ToastStyle {
    ...
    fileprivate var backgroundColor: UIColor = UIColor.black.withAlphaComponent(0.8)
    fileprivate var titleColor: UIColor = .white
    fileprivate var messageColor: UIColor = .white
    ...
    fileprivate var horizontalPadding: CGFloat = 10.0
    fileprivate var verticalPadding: CGFloat = 10.0
    fileprivate var cornerRadius: CGFloat = 10.0;
    fileprivate var titleFont: UIFont = .boldSystemFont(ofSize: 16.0)
    fileprivate var messageFont: UIFont = .systemFont(ofSize: 16.0)
    fileprivate var titleAlignment: NSTextAlignment = .left
    fileprivate var messageAlignment: NSTextAlignment = .left
    fileprivate var titleNumberOfLines = 0
    fileprivate var messageNumberOfLines = 0
    ...
    fileprivate var imageSize = CGSize(width: 80.0, height: 80.0)
